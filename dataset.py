...
class CoData(data.Dataset):
    def __init__(self, img_root, dep_root, gt_root, img_size, transform, max_num, is_train):
        ...
        #
        self.dep_dirs = list(
            map(lambda x: os.path.join(dep_root, x), class_list))
    ...
    def __getitem__(self, item):
        ...
        #
        dep_paths = list(
            map(lambda x: os.path.join(self.dep_dirs[item], x[:-4] + '.png'), names))
        gt_paths = list(
        ...
        if self.is_train:
            ...
            #
            new_dep_paths = [dep_paths[i] for i in sampled_list]
            dep_paths = new_dep_paths
        ...
        #
        deps = torch.Tensor(final_num, 1, self.size[0], self.size[1])
        ...
        for idx in range(final_num):
            ...
            dep = Image.open(dep_paths[idx]).convert('L')
            ...
            [img, dep, gt] = self.transform(img, dep, gt)
        ...
        if self.is_train:
            ...
            return imgs, deps, gts, subpaths, ori_sizes, cls_ls
        else:
            return imgs, deps, gts, subpaths, ori_sizes
...
class FixedResize(object):
    ...
    def __call__(self, img, dep, gt):
        # assert img.size == gt.size

        img = img.resize(self.size, Image.BILINEAR)
        dep = dep.resize(self.size, Image.NEAREST)
...
class ToTensor(object):
    def __call__(self, img, dep, gt):
        return F.to_tensor(img), F.to_tensor(dep), F.to_tensor(gt)
...
class RandomScaleCrop(object):
    ...
    def __call__(self, img, dep, mask):
        ...
        dep = dep.resize((ow, oh), Image.NEAREST)
        ...
        if short_size < self.crop_size:
            ...
            dep = ImageOps.expand(dep, border=(0, 0, padw, padh), fill=0)
        ...
        dep = dep.crop((x1, y1, x1 + self.crop_size, y1 + self.crop_size))
...
class RandomRotation(object):
    ...
    def __call__(self, img, dep, gt):
        ...
        return F.rotate(img, angle, Image.BILINEAR, self.expand, self.center), F.rotate(dep, angle, Image.BILINEAR,
                                                                                        self.expand,
                                                                                        self.center), F.rotate(gt,
                                                                                                               angle,
                                                                                                               Image.NEAREST,
                                                                                                               self.expand,
                                                                                                               self.center)
...
def get_loader(img_root, dep_root, gt_root, img_size, batch_size, max_num=float('inf'), istrain=True, shuffle=False,
    ...
    dataset = CoData(img_root, dep_root, gt_root, img_size, transform, max_num, is_train=istrain)
